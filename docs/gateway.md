...
| Settlement | resolve the **Submitted** ref from the supplier's `submitted_ref` (chain-verified against our receipt), else via indexer; `acceptResult`, **await confirm** before returning success; sweeper is backstop |
...
Routing runs first, outside the mutex; every other step runs inside the per-key `mutex.run(...)`. **Every wait derives from the matched advert's `max_processing_ms`** (the SLA the supplier committed to on chain): the SDK waits on the supplier until `deliver_by + 90 s` (`supplierBudgetMs`), and the mutex run deadline is `oneShotBudgetMs(sla)` = pre-post slack + the SDK's submit budget + Submitted-ref resolution (180 s) + Accept confirmation (120 s) + wallet re-shape. A fixed budget shorter than the SLA fails jobs the supplier finishes inside its window and then settles them as a zombie (2026-09-05/06/07 Apify QA failures).
1. **Route** (`selectSupplier`, before the mutex): indexer rows where `model` matches, `capability_id==="llm.text.generate.v1"`, `advert_status==="Active"`, live `status ∈ {free, unknown}`. Pick an available one (not cheapest). No match → 404 `model_not_found` (`type: invalid_request_error`).
...
3. **Execute**: `submitPrompt({advertRef, messages, payment_lovelace:price, max_output_tokens})` (posts escrow → supplier → verifies receipt). The supplier's done payload carries `submitted_ref` (its Submit tx output).
4. **Settle**: resolve the **Submitted** escrow ref — first from `submitted_ref`, accepted only when the on-chain datum is a Submitted escrow of this wallet whose `result_receipt_hash` equals the hash of the signed receipt we hold; otherwise poll the indexer (mirror buyer `/v1/accept`: match by buyer pkh + `posted_at`, 180 s budget). Then `acceptResult(submittedRef)`, **await confirmation**. On 409/offline mid-route, retry next matching supplier (bounded). On any post-escrow failure, record + leave for the sweeper (don't block the response on reclaim).
...
- Ticket caveats (accepted, demo-grade): a supplier restart forgets used tickets, so an unexpired un-reclaimed escrow ref could reopen a session (bounded by `deliver_by` + the sweep). `/account.locked_in_escrow_lovelace` chain-verifies each "active" indexer row is still unspent before counting it (the indexer never updates spent rows); past 32 active rows the rest count as locked unverified.
...
- **Sweeper** (`SWEEPER_INTERVAL_MS`, default 60 s; scope = the gateway's own custodial keys, via the indexer's per-buyer escrow rows): per key, classify escrows. Open/Claimed past `deliver_by` (+60 s) and older than 11 min (35 min for chat sessions) → `reclaim`. Submitted → retry `acceptResult` in-window. Submitted past accept-window → leave for supplier Release (log; the supplier resolves it with `tx:release-escrows`). Never blind-`reclaim` Submitted. Terminal spends never update indexer rows, so settled refs are remembered in-process to avoid retrying them every tick.
